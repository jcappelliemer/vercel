...
function mapProdotto(wp) {
  ...
  return {
    slug: normalizedSlug,
    nome: fallback?.nome || decodeHtml(wp.title.rendered),
    descrizione,
    categoria: m.categoria || fallback?.categoria || '',
    sottocategoria: m.sottocategoria || fallback?.sottocategoria || '',
    applicazione: m.applicazione || fallback?.applicazione || '',
    certificazione: m.certificazione || fallback?.certificazione || '',
    garanzia: m.garanzia || fallback?.garanzia || '',
    tipoVetro: m.tipo_vetro || fallback?.tipoVetro || '',
    specificheTecniche,
